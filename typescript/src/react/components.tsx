...
import { useBalance, useEvents, useTransaction } from './hooks';
import { SelendraContext, useSelendraContext } from './provider';
...
export function SelendraProvider({
  ...
  const connect = useCallback(async (config: any) => {
    try {
      ...
      const newSdk = new SelendraSDK(config);
  ...
  const switchChain = useCallback(async (newChainType: ChainType) => {
...
const ThemeContext = createContext<{
  ...
  mode: 'light' | 'dark';
  toggleMode: () => void;
  setTheme: (theme: Partial<SelendraTheme>) => void;
} | null>(null);

const defaultTheme: SelendraTheme = {
  colors: {
    primary: '#6366f1',
    secondary: '#8b5cf6',
    success: '#10b981',
...
export function ThemeProvider({
  ...
  const [theme, setTheme] = useState<SelendraTheme>(() => ({
    ...defaultTheme,
    ...customTheme
  }));
  ...
  const updateTheme = useCallback((newTheme: Partial<SelendraTheme>) => {
    setTheme(prev => ({
      ...prev,
      ...newTheme,
...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError && this.state.error) {
      ...
      const resetError = () => super.setState({ hasError: false, error: null, errorInfo: null });
      return <FallbackComponent error={this.state.error} retry={resetError} />;
    }
...
export function WalletConnector({
  ...
  style,
  ...props
}: WalletConnectorProps): React.ReactElement {
  const { sdk, isConnected, chainType: account, connect, disconnect } = useContext(SelendraContext)!;
  ...
  const handleConnect = useCallback(async (walletType: string) => {
    try {
      ...
      const config = {
        chainType: 'substrate' as ChainType, // or detect based on wallet
        endpoint: 'wss://rpc.selendra.org',
        walletType
      };
  ...
  ].filter(wallet => wallets.includes(wallet.id as any));
  ...
  return (
    <div className={className} style={{ position: 'relative', ...style }} {...props}>
      <button
        onClick={() => showWalletList && setShowDropdown(!showDropdown)}
...
export function BalanceDisplay({
  ...
  const { theme } = useTheme();
  const { balance, formatted, isLoading, error: balanceError } = useBalance(address);
...
export function TransactionButton({
  ...
  style,
  ...props
}: TransactionButtonProps): React.ReactElement {
  const { theme } = useTheme();
...
export function ConnectionStatus({
  ...
  const { theme } = useTheme();
  const { isConnected, chainType, error, account } = useContext(SelendraContext)!;
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
        ...
        {showBalance && balanceInfo && (
          <div style={{
            ...
            {(Number(balanceInfo.total) / Math.pow(10, balanceInfo.decimals || 18)).toFixed(4)} {balanceInfo.symbol}
          </div>
        )}
      </div>
    </div>
...
export function EventList({
  ...
  const { theme } = useTheme();
  const { events: hookEvents, isLoading } = useEvents();
  ...
  const defaultEventRenderer = (event: EventData, index: number) => (
    <div
      key={`${event.id}-${index}`}
      style={{
        padding: theme.spacing.md,
  ...
  return (
    ...
    >
      {filteredEvents.map((event, index) =>
        renderEvent ? renderEvent(event, index) : defaultEventRenderer(event, index)
      )}
    </div>
  );
}
...
export function LoadingSpinner({ size = 'md', className, style }: {
  size?: 'sm' | 'md' | 'lg';
  className?: string;
