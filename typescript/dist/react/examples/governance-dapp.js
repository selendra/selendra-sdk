...
exports.GovernanceDApp = GovernanceDApp;
exports.GovernanceDAppApp = GovernanceDAppApp;
...
function ProposalCard({ proposal, onVote }) {
    ...
    const [hasVoted, setHasVoted] = (0, react_1.useState)(false);
    const [selectedVote, setSelectedVote] = (0, react_1.useState)(null);
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            ...
            <p className="text-gray-600 text-sm">
              {isExpanded ? proposal.description : (0, utils_1.truncateString)(proposal.description, 200)}
            </p>
            {proposal.description.length > 200 && (<button onClick={() => setIsExpanded(!isExpanded)} className="text-blue-600 hover:text-blue-800 text-sm mt-1">
        ...
        <div className="mb-4">
          <div className="flex justify-between text-sm mb-2">
            <span>For: {(0, utils_1.formatBalance)({ balance: proposal.votesFor, decimals: 18, symbol: 'SEL' })}</span>
            <span>Against: {(0, utils_1.formatBalance)({ balance: proposal.votesAgainst, decimals: 18, symbol: 'SEL' })}</span>
            <span>Abstain: {(0, utils_1.formatBalance)({ balance: proposal.votesAbstain, decimals: 18, symbol: 'SEL' })}</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2 flex">
            <div className="bg-green-500 h-2 rounded-l-full" style={{ width: `${votesForPercent}%` }}/>
            <div className="bg-red-500 h-2" style={{ width: `${votesAgainstPercent}%` }}/>
            <div className="bg-gray-400 h-2 rounded-r-full" style={{ width: `${votesAbstainPercent}%` }}/>
          </div>
        </div>

        {/* Time Remaining */}
        <div className="flex items-center justify-between mb-4 text-sm text-gray-500">
          <span>Created by {(0, utils_1.formatAddress)(proposal.author, 6)}</span>
          {!isExpired ? (<span className="text-yellow-600">
              Ends in {(0, utils_1.formatTimestamp)(proposal.endsAt, { relative: true })}
            </span>) : (<span className="text-red-600">Voting ended</span>)}
        </div>

        {/* Voting Interface */}
        {proposal.status === 'active' && !isExpired && !hasVoted && (<div className="border-t pt-4">
            <h4 className="font-medium mb-3">Cast Your Vote</h4>
            <div className="flex space-x-2 mb-3">
              <button onClick={() => setSelectedVote('aye')} className={`flex-1 py-2 px-3 rounded-lg border-2 ${selectedVote === 'aye'
                ? 'border-green-500 bg-green-50 text-green-700'
                : 'border-gray-300 hover:border-green-300'}`}>
                👍 For
              </button>
              <button onClick={() => setSelectedVote('nay')} className={`flex-1 py-2 px-3 rounded-lg border-2 ${selectedVote === 'nay'
                ? 'border-red-500 bg-red-50 text-red-700'
                : 'border-gray-300 hover:border-red-300'}`}>
                👎 Against
              </button>
              <button onClick={() => setSelectedVote('abstain')} className={`flex-1 py-2 px-3 rounded-lg border-2 ${selectedVote === 'abstain'
                ? 'border-gray-500 bg-gray-50 text-gray-700'
                : 'border-gray-300 hover:border-gray-400'}`}>
                🤷 Abstain
              </button>
            </div>
            <button onClick={handleVote} disabled={!selectedVote} className="w-full py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed">
              Submit Vote
            </button>
          </div>)}

        {hasVoted && (<div className="border-t pt-4">
        ...
        <div className="flex items-center justify-between text-sm text-gray-500 border-t pt-4 mt-4">
          <span>Threshold: {(0, utils_1.formatBalance)({ balance: proposal.threshold, decimals: 18, symbol: 'SEL' })}</span>
          <button onClick={() => window.open((0, utils_1.getExplorerUrl)('substrate', proposal.id), '_blank')} className="text-blue-600 hover:text-blue-800">
            View Details
          </button>
        </div>
      </div>
...
function CreateProposal({ onProposalCreated }) {
    ...
    const [type, setType] = (0, react_1.useState)('council');
    const [amount, setAmount] = (0, react_1.useState)('');
    const [beneficiary, setBeneficiary] = (0, react_1.useState)('');
    ...
    return (<div className="bg-white rounded-lg shadow-md p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Brief description of your proposal"/>
        </div>
        ...
        <div>
          ...
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} required rows={4} className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Detailed explanation of your proposal"/>
        </div>
        ...
        {type === 'treasury' && (<>
            <div>
              ...
              <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} required step="0.001" className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0.0"/>
            </div>
            ...
            <div>
              ...
              <input type="text" value={beneficiary} onChange={(e) => setBeneficiary(e.target.value)} required className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="0x..."/>
            </div>
          </>)}
        ...
        <div className="flex space-x-3">
          ...
          <button type="submit" disabled={isSubmitting || !title || !description} className="flex-1 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed">
...
function CouncilOverview() {
    const [councilMembers, setCouncilMembers] = (0, react_1.useState)([]);
    ...
    if (isLoading) {
        return <components_1.LoadingSkeleton variant="list" lines={5}/>;
    }
    const totalVotingPower = councilMembers.reduce((sum, member) => sum + parseFloat(member.votingPower), 0);
    return (<div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="mb-6">
        ...
        <div className="text-sm text-gray-600">
          Total Voting Power: {(0, utils_1.formatBalance)({ balance: totalVotingPower.toString(), decimals: 18, symbol: 'SEL' })}
        </div>
      </div>

      <div className="space-y-4">
        {councilMembers.map((member, index) => (<div key={index} className="flex items-center justify-between p-4 border rounded-lg">
            <div className="flex items-center space-x-3">
              <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center">
                <span className="text-lg font-bold text-blue-600">
                  {member.name?.[0] || (0, utils_1.formatAddress)(member.address, 2)[0]}
                </span>
              </div>
              <div>
                <div className="font-medium text-gray-900">
                  {member.name || (0, utils_1.formatAddress)(member.address, 8)}
                </div>
                <div className="text-sm text-gray-500">
                  Voting Power: {(0, utils_1.formatBalance)({ balance: member.votingPower, decimals: 18, symbol: 'SEL' })}
                </div>
              </div>
            </div>
            <div className="text-right">
              {member.lastVote && (<div className="text-sm text-gray-500">
                  Last vote: {(0, utils_1.formatTimestamp)(member.lastVote, { relative: true })}
                </div>)}
...
function TreasuryOverview() {
    const [treasuryBalance, setTreasuryBalance] = (0, react_1.useState)('0');
    ...
    return (<div className="bg-white rounded-lg shadow-md p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="text-center p-4 bg-blue-50 rounded-lg">
          <div className="text-2xl font-bold text-blue-600">
            {(0, utils_1.formatBalance)({ balance: treasuryBalance, decimals: 18, symbol: 'SEL' })}
          </div>
        ...
        <div className="text-center p-4 bg-green-50 rounded-lg">
          <div className="text-2xl font-bold text-green-600">
            {(0, utils_1.formatBalance)({ balance: totalRequested.toString(), decimals: 18, symbol: 'SEL' })}
          </div>
      ...
      <div>
        ...
        <div className="space-y-3">
          {proposals.slice(0, 5).map((proposal) => (<div key={proposal.id} className="flex items-center justify-between p-4 border rounded-lg">
              <div className="flex-1">
                <div className="font-medium text-gray-900 mb-1">
                  {proposal.reason}
                </div>
                <div className="text-sm text-gray-500">
                  To: {(0, utils_1.formatAddress)(proposal.beneficiary, 6)}
                </div>
                <div className="text-sm text-gray-500">
                  Proposed by {(0, utils_1.formatAddress)(proposal.proposedBy, 6)}
                </div>
              </div>
              <div className="text-right">
                <div className="font-medium text-gray-900">
                  {(0, utils_1.formatBalance)({ balance: proposal.amount, decimals: 18, symbol: 'SEL' })}
                </div>
                <div className={`text-sm px-2 py-1 rounded-full inline-block ${proposal.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
 ...
 */
function GovernanceDApp({ showProposals = true, showVoting = true, showTreasury = true, showCouncil = true, className, style }) {
    const { isConnected, account } = (0, hooks_1.useSelendraSDK)();
    const [activeTab, setActiveTab] = (0, react_1.useState)('proposals');
    const [proposals, setProposals] = (0, react_1.useState)([]);
    ...
    (0, react_1.useEffect)(() => {
        const loadProposals = async () => {
            try {
                ...
                const mockProposals = Array.from({ length: 15 }, (_, i) => ({
                    id: (i + 1).toString(),
                    ...
                    type: ['treasury', 'council', 'democracy', 'technical'][i % 4],
                    author: `0x${Math.random().toString(16).substr(2, 40)}`,
                    status: ['active', 'pending', 'executed', 'rejected'][i % 4],
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {activeTab === 'proposals' && (<div>
            ...
            {isLoading ? (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {Array.from({ length: 6 }).map((_, i) => (<components_1.LoadingSkeleton key={i} variant="card"/>))}
              </div>) : filteredProposals.length === 0 ? (<div className="text-center py-12">
                ...
                {filteredProposals.map((proposal) => (<ProposalCard key={proposal.id} proposal={proposal} onVote={handleVote}/>))}
              </div>)}
          </div>)}

        {activeTab === 'create' && (<CreateProposal onProposalCreated={handleProposalCreated}/>)}

        {activeTab === 'treasury' && showTreasury && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <TreasuryOverview />
        ...
        {activeTab === 'council' && showCouncil && (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <CouncilOverview />
            <div className="bg-white rounded-lg shadow-md p-6">
 ...
 */
function GovernanceDAppApp(props) {
...
exports.default = GovernanceDAppApp;
//# sourceMappingURL=governance-dapp.js.map