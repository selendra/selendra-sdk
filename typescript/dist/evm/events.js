...
exports.EventParser = exports.EventFilters = exports.EventSubscription = exports.EventManager = void 0;
...
class EventManager extends events_1.EventEmitter {
    constructor(provider) {
        ...
        Object.defineProperty(this, "nextSubscriptionId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 1
     ...
     */
    async subscribeContract(contract, eventName, args = [], options = {}) {
     ...
     */
    async getTransactionEvents(transactionHash, contracts) {
     ...
     */
    setupSubscriptionEvents(subscription) {
    ...
    encodeTopicValue(value, type) {
        ...
        if (type === 'bytes32') {
            return typeof value === 'string' ? value : '0x' + Buffer.from(String(value)).toString('hex').padEnd(64, '0');
        }
...
class EventSubscription extends events_1.EventEmitter {
    constructor(id, filter, provider, options = {}) {
        super();
    ...
    setupWebSocketEvents() {
        ...
        this.provider.on(this.subscriptionId, (log) => {
            this.handleLog(log);
        });
    }
    /**
...
class EventFilters {
     ...
     */
    static blockRange(fromBlock, toBlock) {
        return {
            fromBlock,
            toBlock
        };
    ...
    static transferEvents(from, to) {
        ...
        if (from !== undefined) {
            topics.push(from ? [from] : null);
        }
        else {
            topics.push(null);
        }
        if (to !== undefined) {
            topics.push(to ? [to] : null);
        }
        else {
            topics.push(null);
        }
     ...
     */
    static approvalEvents(owner, spender) {
        ...
        const topics = [approvalSignature];
        if (owner !== undefined) {
            topics.push(owner ? [owner] : null);
        }
        else {
            topics.push(null);
        }
        if (spender !== undefined) {
            topics.push(spender ? [spender] : null);
        }
        else {
            topics.push(null);
        }
            ...
            if (filter.address) {
                if (combined.address) {
                    if (Array.isArray(combined.address) && Array.isArray(filter.address)) {
                        ...
                        combined.address.push(filter.address);
                    }
                ...
                for (let i = 0; i < Math.max(combined.topics.length, filter.topics.length); i++) {
                    ...
                    if (topic1 && topic2) {
                        ...
                        if (Array.isArray(topic1) && Array.isArray(topic2)) {
                            ...
                            combined.topics[i] = topic1.includes(topic2) ? topic1 : [...topic1, topic2];
                        }
                        else if (Array.isArray(topic2)) {
                            combined.topics[i] = topic2.includes(topic1) ? topic2 : [topic1, ...topic2];
                        }
                        else if (topic1 !== topic2) {
                            combined.topics[i] = [topic1, topic2];
                        }
                    }
                    else if (topic1) {
...
class EventParser {
     ...
     */
    static createSignature(eventName, inputs) {
     ...
     */
    static decodeTopicValue(topic, type) {
     ...
     */
    static encodeTopicValue(value, type) {
        if (value === null || value === undefined) {
            return null;
        }
        if (type === 'address') {
        ...
        if (type.startsWith('bytes')) {
            const bytes = typeof value === 'string' && value.startsWith('0x') ? value.slice(2) : String(value);
