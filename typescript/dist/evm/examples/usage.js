...
exports.basicClientSetup = basicClientSetup;
exports.walletOperations = walletOperations;
...
async function basicClientSetup() {
    ...
    const client = (0, index_1.createEvmClient)({
        network: 'mainnet',
        debug: true
    });
...
async function walletOperations() {
    ...
    const wallet = index_1.SelendraWallet.createRandom();
    console.log('New wallet address:', wallet.getAddress());
    // Create wallet from private key
    const privateKey = '0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef';
    const walletFromKey = new index_1.SelendraWallet(privateKey);
...
async function transactionManagement() {
    ...
    const txBuilder = index_1.TransactionBuilder.transfer(wallet.getAddress(), '0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6', '0.1');
    ...
    console.log('Estimated cost:', (0, index_1.weiToEther)(gasEstimate.estimatedCost), 'ETH');
...
async function contractInteraction() {
    ...
    const tokenERC20 = new index_1.ERC20Contract(tokenAddress, client, connectedWallet);
    try {
        ...
        console.log('Total supply:', (0, index_1.formatBalance)(totalSupply, Number(decimals), symbol));
        // Get balance
        const balance = await tokenERC20.balanceOf(wallet.getAddress());
        console.log('Wallet balance:', (0, index_1.formatBalance)(balance, Number(decimals), symbol));
        // Contract deployment example
        const contractFactory = client.getContractFactory(erc20ABI, '0x608060405234801561001057600080fd5b50');
...
async function eventManagement() {
    console.log('\n=== Event Management ===');
    const client = (0, index_1.createEvmClient)();
    const eventManager = new index_1.EventManager(client);
    ...
    try {
        ...
        if (client instanceof index_1.WebSocketProvider) {
 ...
 */
async function webSocketProvider() {
    console.log('\n=== WebSocket Provider ===');
    try {
        // Create WebSocket provider
        const wsProvider = (0, index_1.createWebSocketProvider)({
            network: 'mainnet',
            debug: true
        });
...
async function advancedConfiguration() {
    ...
    const customClient = (0, index_1.createEvmClient)({
        network: 'mainnet',
        timeout: 60000,
        maxRetries: 5,
        debug: true,
    ...
    console.log('Current gas price:', (0, index_1.weiToEther)(gasPrice), 'ETH');
    const maxFeePerGas = await customClient.getMaxFeePerGas();
    console.log('Max fee per gas:', (0, index_1.weiToEther)(maxFeePerGas), 'ETH');
    const maxPriorityFeePerGas = await customClient.getMaxPriorityFeePerGas();
    console.log('Max priority fee per gas:', (0, index_1.weiToEther)(maxPriorityFeePerGas), 'ETH');
}
/**
...
async function batchOperations() {
    ...
    try {
        ...
        console.log('Gas price:', (0, index_1.weiToEther)(results[2]), 'ETH');
        console.log('Balance:', (0, index_1.weiToEther)(results[3]), 'ETH');
    }
    catch (error) {
