...
exports.testUtils = {
    ...
    generateMockAddress: (prefix = '0x') => {
        const randomBytes = Array.from({ length: 20 }, () => Math.floor(Math.random() * 256).toString(16).padStart(2, '0')).join('');
    ...
    generateMockSignature: () => {
        const randomBytes = Array.from({ length: 65 }, () => Math.floor(Math.random() * 256).toString(16).padStart(2, '0')).join('');
        return '0x' + randomBytes;
    },
    /**
    ...
    createMockTransactionResult: (hash) => ({
        hash: hash || exports.testUtils.generateMockHash(),
        status: 'finalized',
        blockHash: exports.testUtils.generateMockHash(),
        blockNumber: Math.floor(Math.random() * 1000000),
        fee: exports.testUtils.generateMockBalance(),
    }),
    /**
    ...
    createMockNetworkStatus: () => ({
        ...
        blockNumber: Math.floor(Math.random() * 1000000),
        blockHash: exports.testUtils.generateMockHash(),
        genesisHash: exports.testUtils.generateMockHash(),
        isSyncing: false,
        timestamp: Date.now(),
    }),
};
