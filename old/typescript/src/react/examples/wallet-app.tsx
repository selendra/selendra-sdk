...
import { WalletTemplateProps } from '../types';

interface Transaction {
  hash: string;
  type: 'send' | 'receive' | 'contract' | 'stake' | 'unstake';
  from: string;
  to: string;
  amount: string;
  symbol: string;
...
function SendTransaction({ onTransactionSent }: {
  onTransactionSent: (tx: Transaction) => void;
}) {
  const { account } = useSelendraSDK();
  const balance = useBalance();
  const [recipient, setRecipient] = useState('');
  const [amount, setAmount] = useState('');
  const [selectedToken, setSelectedToken] = useState('SEL');
  ...
  const validateForm = () => {
    ...
    const addressValidation = validateAddress(recipient, 'substrate');
    if (!addressValidation.isValid) {
      newErrors.recipient = addressValidation.errors[0];
    }

    const amountValidation = validateAmount(
      (parseFloat(amount || '0') * Math.pow(10, 18)).toString(),
      balance.balance!,
      18
    );
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex space-x-2">
            ...
            <button
              onClick={() => setAmount(
                (parseFloat(balance.balance?.balance || '0') / Math.pow(10, 18) * 0.5).toString()
              )}
            ...
            <button
              onClick={() => setAmount(
                (parseFloat(balance.balance?.balance || '0') / Math.pow(10, 18)).toString()
              )}
              className="px-3 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-sm"
            >
              Max
          ...
          <div className="text-sm text-gray-500 mt-1">
            Available: {formatBalance(balance.balance!)}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex space-x-2">
            <input
              ...
              readOnly
              className="flex-1 px-3 py-2 border bg-gray-50 rounded-lg font-mono text-sm"
            />
            <button
...
function TransactionHistory() {
    ...
    const loadTransactions = async () => {
      try {
        ...
        const mockTransactions: Transaction[] = Array.from({ length: 20 }, (_, i) => ({
          ...
          status: ['pending', 'included', 'finalized', 'failed'][Math.floor(Math.random() * 4)] as any,
  ...
  if (isLoading) {
    return <LoadingSkeleton variant="list" lines={10} />;
  }
      ...
      {filteredTransactions.length === 0 ? (
        ...
        <div className="space-y-3">
          {filteredTransactions.map((tx) => (
            <div key={tx.hash} className="flex items-center space-x-3 p-3 hover:bg-gray-50 rounded-lg">
              ...
              <div className="flex-1">
                ...
                <div className="text-sm text-gray-500">
                  {formatTimestamp(tx.timestamp, { relative: true })}
                </div>
              ...
              <div className="text-right">
                <div className="font-medium">
                  {tx.type === 'send' ? '-' : '+'}
                  {formatBalance({ balance: tx.amount, symbol: tx.symbol, decimals: 18 } as any)}
                </div>
                <button
                  onClick={() => window.open(getExplorerUrl('substrate', tx.hash), '_blank')}
...
export function WalletApp({
  ...
  className,
  style
}: WalletTemplateProps): JSX.Element {
 ...
 */
export function WalletAppApp(props: WalletTemplateProps): JSX.Element {
  return (
    <ErrorBoundary>
      <ThemeProvider>
        <SelendraProvider
