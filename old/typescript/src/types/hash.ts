...
import type { Hash, BlockHash, TransactionHash, StateRoot } from './common';
...
export class HashUtils {
  ...
  static createMerkleTree(hashes: Hash[]): {
    ...
    const level0: MerkleNode[] = hashes.map((hash, index) => ({
      hash,
      level: 0,
      index,
    }));

    const tree: MerkleNode[][] = [level0];
    ...
    while (currentLevel.length > 1) {
      ...
      for (let i = 0; i < currentLevel.length; i += 2) {
        ...
        const nodeHash = `0x${combinedHash.slice(2, 66)}` as Hash;
  ...
  static generateMerkleProof(leafHash: Hash, tree: MerkleNode[][]): MerkleProof {
    ...
    const path: Array<{ hash: Hash; isLeft: boolean }> = [];
  ...
  static verifyMerkleProof(proof: MerkleProof): boolean {
    ...
    for (const sibling of proof.path) {
      ...
      // Simple synchronous hash combination (placeholder)
      currentHash = `0x${combinedHash.slice(2, 66)}` as Hash;
    }
  ...
  private static async keccak256(data: Uint8Array): Promise<Uint8Array> {
  ...
  private static async blake2_256(data: Uint8Array): Promise<Uint8Array> {
