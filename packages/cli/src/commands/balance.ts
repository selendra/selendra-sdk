...
export async function balanceCommand(address: string, options: BalanceOptions) {
  ...
  try {
    ...
    let substrateBalance: { free: string; reserved: string; frozen: string } | null = null;
    ...
    if (evmBalance !== null) {
      ...
      printKeyValue('Balance:', `${formatBalance(evmBalance)} SEL`, chalk.green);
    ...
    if (substrateBalance) {
      ...
      printKeyValue('Free:', `${formatBalance(substrateBalance.free)} SEL`, chalk.green);
      printKeyValue('Reserved:', `${formatBalance(substrateBalance.reserved)} SEL`, chalk.yellow);
      printKeyValue('Frozen:', `${formatBalance(substrateBalance.frozen)} SEL`, chalk.gray);
      
      const total = BigInt(substrateBalance.free) + BigInt(substrateBalance.reserved);
