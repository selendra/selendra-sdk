...
import { isAddress, toHex, type Hex, type Address } from "viem";
...
export class SelendraWallet {
  ...
  constructor(privateKey: string, type: WalletType = "evm") {
    ...
    if (type === "evm") {
      // Create EVM account using viem
      const key = (
        privateKey.startsWith("0x") ? privateKey : `0x${privateKey}`
      ) as Hex;
      this.evmPrivateKey = key;
  ...
  static createRandom(type: WalletType = "evm"): SelendraWallet {
    if (type === "evm") {
      ...
      return new SelendraWallet(privateKey, "evm");
    } else {
  ...
  static fromMnemonic(
    ...
    if (type === "evm") {
      ...
      const hdAccount = mnemonicToAccount(mnemonic, { path: path as any });
      // We need to get the private key - use a workaround
      const wallet = new SelendraWallet("0x" + "00".repeat(32), "evm");
      // Cast to PrivateKeyAccount since HD accounts have the same interface
      wallet.evmAccount = hdAccount as unknown as PrivateKeyAccount;
      // Note: viem doesn't expose private key from mnemonic account directly
      // This is a limitation - for full private key access, use generatePrivateKey
      return wallet;
    } else {
  ...
  static async fromEncryptedJson(
    ...
    if (type === "evm") {
      // For EVM, we only support Substrate-style JSON for now
      // viem doesn't have built-in encrypted JSON support like ethers
      throw new Error(
        "Encrypted JSON import for EVM wallets is not supported. Use private key or mnemonic instead."
      );
    } else {
  ...
  getAddress(ss58Format: number = 204): string {
    if (this.walletType === "evm") {
      if (!this.evmAccount) throw new Error("EVM wallet not initialized");
      return this.evmAccount.address;
    } else {
      if (!this.substrateAccount)
        throw new Error("Substrate account not initialized");
  ...
  getPrivateKey(): string {
    if (this.walletType === "evm") {
      if (!this.evmPrivateKey)
        throw new Error(
          "EVM private key not available (wallet may have been created from mnemonic)"
        );
      ...
      if (!secretKey) {
        throw new Error(
          "Cannot extract private key from this Substrate account (may be derived from mnemonic)"
        );
      }
  ...
  getPublicKey(): string {
    if (this.walletType === "evm") {
      if (!this.evmAccount) throw new Error("EVM wallet not initialized");
      // viem accounts don't expose public key directly, derive from address
      return this.evmAccount.address;
    } else {
      if (!this.substrateAccount)
        throw new Error("Substrate account not initialized");
   ...
   * @param provider - EVM public client or Substrate API
  ...
  async getBalance(provider: any, blockTag?: string | number): Promise<bigint> {
    if (this.walletType === "evm") {
      if (!this.evmAccount) throw new Error("EVM wallet not initialized");
      // Assume provider is a viem PublicClient
      ...
      if (!this.substrateAccount)
        throw new Error("Substrate account not initialized");
      const accountInfo = await provider.query.system.account(
        this.substrateAccount.address
      );
  ...
  async getTransactionCount(
    ...
    if (this.walletType === "evm") {
      ...
      if (!this.substrateAccount)
        throw new Error("Substrate account not initialized");
      const nonce = await provider.query.system.account(
        this.substrateAccount.address
      );
  ...
  async signMessage(message: string | Uint8Array): Promise<string> {
    if (this.walletType === "evm") {
      ...
      return await this.evmAccount.signMessage({ message: msg });
    } else {
      if (!this.substrateAccount)
        throw new Error("Substrate account not initialized");
      const messageBytes =
  ...
  async signTypedData(
    ...
    return await this.evmAccount.signTypedData({
  ...
  async encrypt(
    ...
    if (this.walletType === "evm") {
      // viem doesn't have built-in encryption, throw error for now
      throw new Error(
        "EVM wallet encryption is not supported. Use Substrate account or store private key securely."
      );
    } else {
      if (!this.substrateAccount)
        throw new Error("Substrate account not initialized");
  ...
  /**
   * Get the underlying EVM account (if applicable)
   * @returns PrivateKeyAccount or undefined
   */
  getEvmAccount(): PrivateKeyAccount | undefined {
...
export class WalletUtils {
  ...
  static async decryptKey(
    ...
    return await SelendraWallet.fromEncryptedJson(
      encryptedJson,
      password,
      type
    );
  ...
  static deriveEvmAddress(substrateAddress: string | KeyringPair): string {
    ...
    if (!secretKey) {
      throw new Error(
        "Cannot derive EVM address: secretKey not available from Substrate account"
      );
    }

    const privateKey = ("0x" + Buffer.from(secretKey).toString("hex")) as Hex;
  ...
  static formatAddress(
    ...
    return `${address.slice(0, prefixLength)}...${address.slice(
      -suffixLength
    )}`;
  }
}
