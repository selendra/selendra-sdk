...
export class UnifiedAccountsManager {
  ...
  async initialize(): Promise<void> {
    ...
    this.signatureHelper = new UnifiedAccountSignature(
      this.chainId,
      genesisHash
    );
  }
  ...
  private ensureApi(): ApiPromise {
    ...
    if (!api) {
      throw new Error(
        "Substrate provider not connected. Call sdk.connect() first."
      );
    }
    return api;
  }
   ...
   */
  async getEvmAddressForSubstrate(
    accountId: string
  ): Promise<EvmMappingResult> {
    const api = this.ensureApi();

    // Query NativeToEvm storage
    const mapped = (await api.query.unifiedAccounts.nativeToEvm(
      accountId
    )) as any;
   ...
   */
  async getSubstrateAddressForEvm(
    evmAddress: string
  ): Promise<SubstrateMappingResult> {
    const api = this.ensureApi();

    // Query EvmToNative storage
    const mapped = (await api.query.unifiedAccounts.evmToNative(
      evmAddress
    )) as any;

    // Calculate default address
    const defaultAddress = calculateDefaultSubstrateAddress(
      evmAddress,
      this.ss58Prefix
    );
  ...
  async claimEvmAddress(
    ...
    const evmAccount = privateKeyToAccount(
      actualEvmPrivateKey as `0x${string}`
    );
  ...
  async claimDefaultEvmAddress(
    ...
    const defaultEvmAddress = this.getDefaultEvmAddress(
      substrateAccount.address
    );

    // Sign and send
    return this.sendClaimTransaction(
      tx,
      substrateAccount,
      defaultEvmAddress,
      options
  ...
  async checkClaimEligibility(address: string): Promise<EligibilityCheck> {
    ...
    if (hasMapping) {
      reasons.push(
        "Address already has a mapping. Mappings are permanent and cannot be changed."
      );
    }
  ...
  private deriveEvmKeyFromSubstrate(account: KeyringPair): string {
    try {
      ...
      if (!seed || seed.length !== 32) {
        throw new Error(
          "Invalid seed length for EVM key derivation (expected 32 bytes)"
        );
      }
      ...
      throw new Error(
        `${UnifiedAccountError.SEED_DERIVATION_FAILED}: ${
  ...
  private async sendClaimTransaction(
    ...
    return new Promise((resolve, reject) => {
      tx.signAndSend(
        account,
        {
          tip: options.tip ? options.tip.toString() : undefined,
          nonce: options.nonce,
        },
  ...
  private async parseClaimResult(
    ...
    for (const { event } of events) {
      ...
      if (
        event.section === "unifiedAccounts" &&
        event.method === "AccountClaimed"
      ) {
        claimEvents.push({
          type: "AccountClaimed",
